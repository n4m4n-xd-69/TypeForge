...
<img src="assets/banner.svg" alt="TypeForge — Type Faster. Play Harder." width="100%" />

<br />

# TypeForge

### **TYPE FASTER.** <span><code style="background: linear-gradient(90deg,#a78bfa,#22d3ee); -webkit-background-clip: text; color: transparent; font-weight:800">PLAY HARDER.</code></span>

**A typing-performance platform for people who type for a living — prose, real code,
and 1v1 combat on a clock nobody can cheat.**

Most typing sites drill prose and stop there — fine, until the day your job is brackets,
semicolons and indentation. **TypeForge drills all three, diagnoses the exact keys costing
you, then lets you prove it against real opponents.**
...
[![Framer Motion](https://img.shields.io/badge/Framer_Motion-11-black?style=for-the-badge&logo=framer&logoColor=white)](https://motion.dev)
...
![Tests](https://img.shields.io/badge/tests-388%20passing-a3e635?style=flat-square)
...
[**Live demo**](https://key-stroke-ai.vercel.app) · [**Features**](#-features) · [**Screenshots**](#-screenshots) · [**Forge AI**](#-forge-ai--your-agentic-coach) · [**Architecture**](#-architecture) · [**Setup**](#-installation) · [**Shortcuts**](#️-keyboard-shortcuts)
...
| **Test your speed** | Real-time multiplayer races — matchmaking or custom PIN rooms |
| **Fight for it** | Shadow Battle — tactical 1v1 word combat with parries, chains and Overdrive finishers |
...
### ⚔️ Shadow Battle — 1v1 word combat

A tactical duel where **every word is a move**. Pick a fighter, pick a rival, and trade
damage at the keyboard:

- **60fps stickman combat** driven entirely by your typing — clean words strike, errors leave you open
- **Parries, chains and Overdrive** — read the rival, build a chain, cash it into a finisher
- **Fight real people or forged bots**, asynchronously or live
- **Forge Rating** ranks, from *Quench* to *Damascus*
...
### Landing — a cinematic product introduction

<img src="assets/screenshots/landing.png" alt="TypeForge cinematic landing hero with exploded mechanical keyboard footage, TYPE FASTER. PLAY HARDER. headline and Start Typing / Enter the Arena CTAs" width="92%" />

<sub>Full-screen typing footage, particle field, staggered headline — the product in three seconds.</sub>

<br /><br />

### Home — your day, over live footage

<img src="assets/screenshots/home.png" alt="TypeForge home dashboard with video greeting card, level panel, weekly rhythm chart and daily missions" width="92%" />

<sub>The greeting card plays the same footage; stats, missions and charts sit beneath it.</sub>
...
<sub>A freshly generated passage, live stats that ease rather than snap, and a caret that tracks the line you are on.</sub>
...
<sub>Binary search with worked examples, complexity, and a step-by-step execution reading — generated for the snippet in front of you.</sub>

<br /><br />

### Forge AI — the agentic coach
...
<img src="assets/screenshots/dashboard.png" alt="Analytics dashboard with WPM growth, accuracy trend, skill radar and personal bests" width="92%" />

<sub>Growth, accuracy, consistency, skill radar and the per-key diagnosis behind them.</sub>
...
<sub>The level ladder, today's three missions, and badges across four tiers.</sub>
...
- **Claude Code-style working state** — a shimmering verb (`Forging…`, `Reading your runs…`,
  `Polishing…`) with per-step checkmarks, so waiting never feels like a void
- **Answers end in actions** — "Drill `; p q`", "30s sprint", "Shadow duel": the next move is a
  button, not a suggestion you have to retype
- **Capability grid** — six disciplines, one click each, from the home screen
- **Regenerate & copy** on every answer, streaming throughout, and full thread history on the chat page

All of it runs on the same hedged-failover AI transport as the code panels — and degrades
honestly (clearly disabled, with the reason) when no key is configured.
...
flowchart LR
    ...
    B --> E{Run finished}
    C --> E
    D --> F1[Battlefield race]
    D --> F2[Shadow Battle]
    F1 --> E
    F2 --> E
    E --> G[Session summary<br/>WPM · accuracy · XP]
    ...
    style A fill:#FF7A2F,stroke:#c25a1a,color:#0A0B0D
    style F2 fill:#8B5CF6,stroke:#6d28d9,color:#ffffff
    style G fill:#1f2937,stroke:#4b5563,color:#e5e7eb
    style M fill:#1f2937,stroke:#4b5563,color:#9ca3af
```

**How it flows.** Everything starts at the landing page, which points at the shortest useful
next action. Practice, Code and the Arena's two combat modes all end in a scored session that
writes to local state. Dashboard and Rewards are pure readers of that state — they compute,
never store. Cloud sync mirrors it when you are signed in, and is a no-op when you are not.
...
flowchart TD
    ...
    style TE fill:#FF7A2F,stroke:#c25a1a,color:#0A0B0D
    style CB fill:#8B5CF6,stroke:#6d28d9,color:#ffffff
...
npm test             # 388 tests across the engine, combat and sync
```
...
| `VITE_SUPABASE_URL` · `VITE_SUPABASE_ANON_KEY` | Accounts, cloud sync and live multiplayer |
...
```bash
node scripts/build-icons.mjs    # Logo.jsx -> favicon, PWA icons, og image
```
...
├── public/
│   ├── brand/                 # Forge AI's animated avatar
│   └── vid/                   # Landing / home background footage + poster
├── scripts/
│   ├── build-icons.mjs        # Derives every icon from one logo shape
│   ├── audit-deadcode.mjs     # Finds unreferenced files, exports and imports
│   └── check-contrast.mjs     # Re-derives every documented contrast figure
├── supabase/migrations/       # Schema, RLS, and server-authoritative scoring
├── src/
│   ├── components/
│   │   ├── brand/             # Logo + ForgeAvatar — single sources of the marks
│   │   ├── charts/            # Recharts wrappers, palette, frames
│   │   ├── layout/            # AppShell, command palette, Forge AI panel
...
│   │   ├── shadow/            # Combat engine, bots, transport, anti-cheat
...
│   └── modules/
│       ├── practice/  code/  arena/  battle/  shadow/
│       ├── dashboard/  achievements/  chat/  landing/
│       └── about/  auth/  admin/  home/  profile/
└── vite.config.js
```
...
| <kbd>Esc</kbd> | Restart the run · leave full screen · close Forge AI | Typing surfaces |
...
- **Reduced motion** — every animation checks `prefers-reduced-motion`: the landing particles
  never mount, the background footage holds on its poster, and the live WPM counter snaps instead of easing
...
- **Decorative elements** — footage, particles and the agent avatar are `aria-hidden`, so
  atmosphere is never announced
